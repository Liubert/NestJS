...
export function registerProjectManagementTools(server: McpServer): void {
  ...
  server.tool(
    ...
    ].join(' '),
    {
      slug: z
        ...
        .describe(
          "Project slug — short, lowercase, hyphen-separated identifier (e.g. 'my-app', 'travis-v2'). Must be unique.",
        ),
      name: z
        .string()
        .optional()
    ...
    async ({ slug, name, namespaces }) => {
      try {
        ...
        const project = await apiPost<ProjectCreated>(
        ...
        if (error instanceof ApiError && error.status === 409) {
          return textResult(
            `Error: A project with slug "${slug}" already exists. Use a different slug or call list_projects to see existing projects.`,
          );
        }
  ...
  server.tool(
    ...
    ].join(' '),
    {
      ...
      namespace: z
        ...
        .regex(
          /^[a-z0-9-]+$/,
          'Namespace slug must be lowercase alphanumeric with dashes',
        )
      ...
      reason: z
        ...
        .min(
          10,
          'Reason must be at least 10 characters — explain why no existing namespace fits',
        )
        .describe(
    ...
    async ({ projectSlug, namespace, reason }) => {
      ...
      try {
        const project = await apiGet<{ namespaces: string[] }>(
      ...
      try {
        ...
        logWrite(
          'create_namespace',
          { projectSlug, namespace, reason, existingNamespaces },
          created,
        );
        return textResult(
          [
  ...
  server.tool(
    ...
    async ({ projectSlug, namespace, env, locale }) => {
      try {
        ...
        let page = 1;
        while (true) {
          const data = await apiGet<EntriesPage>(basePath, {
            page,
            limit: 100,
          });
        ...
        if (allEntries.length === 0) {
          return textResult(
            `Namespace ${projectSlug}/${namespace} [${env}] is empty.`,
          );
        }
        ...
        for (const [loc, map] of Object.entries(result)) {
          ...
          lines.push(
            `[${loc}]: ${count} keys${missing > 0 ? ` (${missing} missing)` : ''}`,
          );
        }
  ...
  server.tool(
    ...
    ].join(' '),
    {
      ...
      namespace: z
        .string()
        .describe(
          'Namespace slug — must already exist (use create_namespace first)',
        ),
      translations: z
        .record(z.string(), z.record(z.string(), z.string()))
        .optional()
        .describe(
          'Inline locale → key → value map. Use this for small payloads. Example: { "en": { "save": "Save" }, "nb-NO": { "save": "Lagre" } }',
        ),
      filePath: z
        .string()
        .optional()
        .describe(
          'Absolute path to a JSON file on disk. Format: { "locale": { "key": "value" } }. Use this for large translation files instead of inline JSON.',
        ),
      contexts: z
      ...
      dryRun: z
        ...
        .default(false)
        .describe(
          'If true, validate and preview what would be imported without writing anything',
        ),
    },
    async ({
      projectSlug,
      ...
      try {
        ...
        if (filePath) {
          ...
          return textResult(
            `Provide either "translations" (inline JSON) or "filePath" (path to JSON file).`,
          );
        }
        ...
        const [project, sandboxStatus] = await Promise.all([
          ...
          dryRun
            ...
            : apiGet<{
              ...
              }>(`/translations/projects/${projectSlug}/sandbox/status`).catch(
                () => null,
              ),
        ]);
        ...
        const sandboxWarning: string | null = (() => {
          ...
          if (sandboxStatus.hasChanges) {
            ...
            return [
              ...
              `   Review with get_translation_diff before importing more, or call reset_sandbox to discard existing changes.`,
            ].join('\n');
          }
          return null;
        })();
        ...
        if (dryRun) {
          const perLocale = requestedLocales.map(
            (l) =>
              `  ${l}: ${Object.keys(resolvedTranslations[l]).length} values`,
          );
          return textResult(
            [
            ...
            try {
              ...
              if (err instanceof ApiError && err.status === 404) {
                try {
                  ...
                  failed++;
                  errors.push(
                    `  ${key}: ${postErr instanceof ApiError ? `${postErr.status} ${postErr.message}` : String(postErr)}`,
                  );
                }
              } else {
                failed++;
  ...
  server.tool(
    ...
    ].join(' '),
    {
      ...
      filePath: z
        ...
        .optional()
        .describe(
          'Absolute path to a JSON file on disk. Format: { "locale": { "key": "value" } }',
        ),
      env: z
        .enum(['sandbox', 'production'])
        .default('sandbox')
    ...
    async ({ projectSlug, namespace, translations, filePath, env }) => {
      try {
        ...
        if (filePath) {
          ...
          return textResult(
            `Provide either "translations" (inline) or "filePath".`,
          );
        }
  ...
  server.tool(
    ...
    ].join(' '),
    {
      ...
      keys: z
        .array(z.string())
        .min(1)
        .describe('List of translation keys to check'),
      env: z
        .enum(['sandbox', 'production'])
        .default('sandbox')
    ...
    async ({ projectSlug, namespace, keys, env }) => {
      try {
        ...
        if (missing.length > 0) {
          ...
          lines.push(
            ``,
            `→ Use set_translation or bulk_import to add missing keys.`,
          );
        } else {
    ...
    async ({ projectSlug, namespace, env }) => {
      try {
        ...
        if (localeCodes.length === 0) {
          return textResult(
            `Project "${projectSlug}" has no locales configured.`,
          );
        }
        ...
        if (allEntries.length === 0) {
          return textResult(
            `Namespace ${projectSlug}/${namespace} [${env}] is empty. No keys to analyze.`,
          );
        }
        ...
        const lines = [
          ...
          ...localeStats.map((s) => {
            const bar =
              '█'.repeat(Math.round(s.pct / 5)) +
              '░'.repeat(20 - Math.round(s.pct / 5));
        ...
        if (incomplete.length > 0) {
          ...
          for (const s of incomplete) {
            if (s.missingKeys.length > 0) {
              ...
              if (s.missingKeys.length > 10)
                lines.push(`    ... and ${s.missingKeys.length - 10} more`);
            }
          }
          lines.push(
            ``,
  ...
  server.tool(
    ...
    ].join(' '),
    {
      ...
      aliases: z
        .array(z.string())
        .describe(
          "List of alias codes for this locale (e.g. ['no', 'nb'] for nb-NO). Pass empty array to clear aliases.",
        ),
    },
  ...
  server.tool(
    ...
    ].join(' '),
    {
      ...
      confirmed: z
        ...
        .default(false)
        .describe(
          'Must be true to execute. Without confirmation, returns a warning.',
        ),
    },
    async ({ projectSlug, code, confirmed }) => {
      ...
      try {
        ...
        return textResult(
          `Deleted locale "${code}" from project "${projectSlug}". All translation values for this locale have been removed.`,
        );
      } catch (error) {
        return errorResult(error);
      }
  ...
  server.tool(
    ...
    async ({ projectSlug, namespace, confirmed }) => {
      ...
      try {
        await apiDelete(
          `/translations/projects/${projectSlug}/namespaces/${namespace}`,
        );
        return textResult(
          `Deleted namespace "${namespace}" from project "${projectSlug}". All keys and values have been removed.`,
        );
      } catch (error) {
        return errorResult(error);
      }
