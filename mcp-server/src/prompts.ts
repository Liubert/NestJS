...
const DEFAULT_SETUP_CONTENT = [
  ...
  "",
  "### Translation: two workflows",
  "",
  "There are two ways to translate. **Agent-local translation is the preferred approach** — it produces higher quality results because you have full context about the codebase, UI, and usage.",
  "",
  "#### Preferred: Agent-local translation",
  "You are a skilled multilingual translator. When you need to translate text:",
  "1. Read existing translations in the namespace to match tone and terminology (`list_translations`)",
  "2. Translate the text yourself — you understand the UI context, the codebase, and the meaning better than a generic AI API",
  "3. Save with `set_translation` (single key) or `bulk_set_locale` (many keys, one locale) or `bulk_import` (many keys, many locales)",
  "4. Run `check_entry_quality` (single key) or `bulk_check_quality` (many keys) to validate — the backend AI will score your translation",
  "5. If quality is red (<60): revise and re-save. If yellow (60-84): review the comment and decide. If green (85+): move on.",
  "",
  "**When to use agent-local:**",
  "- Translating new keys (you see the code context)",
  "- Improving existing translations flagged by quality check",
  "- Small to medium batches (1–50 keys) where you can give each key attention",
  "- When the user asks you to translate something",
  "",
  "#### Alternative: Backend AI translation",
  "Use `ai_translate` / `bulk_translate_and_save` when:",
  "- Bulk-filling a new locale (100+ keys) where speed matters more than per-key attention",
  "- You have no context about the text and the backend AI is equally capable",
  "- The user explicitly asks to use backend/Gemini translation",
  "",
  "**Default behavior:** When the user asks to translate, use agent-local translation unless they specify otherwise or the batch is very large (100+ keys).",
  "",
  "---",
  "",
