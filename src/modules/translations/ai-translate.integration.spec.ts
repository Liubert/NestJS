...
(RUN ? describe : describe.skip)(
  'AI integration — translate + quality (real Gemini)',
  () => {
    ...
    // ─── "Reservation" — without context ─────────────────────────────────────

    describe('"Reservation" WITHOUT context', () => {
      const KEY = 'reservation_no_context';
      ...
      it('translate flags contextNeed as required (no existing translations)', async () => {
        ...
        console.log(
          `[reservation no ctx] contextNeed=${contextInfo[KEY]?.need} reason=${contextInfo[KEY]?.reason}`,
        );
        expect(contextInfo[KEY]?.need).toBe('required');
        expect(contextInfo[KEY]?.reason).toBeTruthy();
      }, 30_000);
